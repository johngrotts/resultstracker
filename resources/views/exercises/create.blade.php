...
@section('content')
	...
	{{ csrf_field() }} {{-- token that protects the form --}}
		<div class="form-group">
    		<label for="workout_id">Attach to Which Active Workout?*</label>
			<select class="form-control" name="workout_id"> 
			<option value=''></option>
			@foreach ($workouts as $workout)
				<option value="{{ $workout->id }}">{{ $workout->id }} - {{ $workout->client->last_name }}, {{ $workout->client->first_name }} - {{ $workout->created_at->format('F d, Y') }}</option>
			@endforeach
			</select>
		</div>
		<div class="form-group">
